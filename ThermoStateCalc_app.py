...
class main_window(QWidget,Ui__frm_StateCalculator):
    def __init__(self):
        ...
        self.addStateGroupBoxes()  # Correct method call to add group boxes for State 1 and State 2
        self.setupStatePropertiesDisplay()
        self.setUnits()
        self.show()
        #code added here

    def addStateGroupBoxes(self):
        self.state1GroupBox = self.createStateGroupBox("State 1")
        self.state2GroupBox = self.createStateGroupBox("State 2")
        self.stateChangeGroupBox = self.createStateGroupBox("State Change")

        self._grp_SpecifiedProperties.layout().addWidget(self.state1GroupBox)
        self._grp_SpecifiedProperties.layout().addWidget(self.state2GroupBox)
        self._grp_SpecifiedProperties.layout().addWidget(self.stateChangeGroupBox)
    ...
    def createStateGroupBox(self, title):
        ...
        # Create and add widgets to layout: combo boxes, line edits, and labels
        property1ComboBox = QtWidgets.QComboBox()
        property1ComboBox.addItems(
            ["Pressure (p)", "Temperature (T)", "Specific Volume (v)", "Internal Energy (u)", "Enthalpy (h)",
             "Entropy (s)", "Quality (x)"])
        property2ComboBox = QtWidgets.QComboBox()
        property2ComboBox.addItems(
            ["Pressure (p)", "Temperature (T)", "Specific Volume (v)", "Internal Energy (u)", "Enthalpy (h)",
             "Entropy (s)", "Quality (x)"])

        property1LineEdit = QtWidgets.QLineEdit()
        property2LineEdit = QtWidgets.QLineEdit()
        property1UnitLabel = QtWidgets.QLabel("Unit 1")
        property2UnitLabel = QtWidgets.QLabel("Unit 2")

        layout.addWidget(property1ComboBox, 0, 0)
        layout.addWidget(property1LineEdit, 0, 1)
        layout.addWidget(property1UnitLabel, 0, 2)
        layout.addWidget(property2ComboBox, 1, 0)
        layout.addWidget(property2LineEdit, 1, 1)
        layout.addWidget(property2UnitLabel, 1, 2)

        return groupBox

    def setupStatePropertiesDisplay(self):
        # Create a horizontal layout for the group box
        layout = QtWidgets.QHBoxLayout()

        # Create group boxes for each state and for the change in properties
        self.grpState1 = QtWidgets.QGroupBox("State 1 Properties")
        self.grpState2 = QtWidgets.QGroupBox("State 2 Properties")
        self.grpStateChange = QtWidgets.QGroupBox("State Change")

        # Create vertical layouts for each group box to add labels
        layoutState1 = QtWidgets.QVBoxLayout()
        layoutState2 = QtWidgets.QVBoxLayout()
        layoutStateChange = QtWidgets.QVBoxLayout()

        # Example labels for each state - you will update these dynamically based on calculations
        lblState1Contents = QtWidgets.QLabel("TBD")
        lblState2Contents = QtWidgets.QLabel("TBD")
        lblStateChangeContents = QtWidgets.QLabel("TBD")

        # Add the labels to their respective layouts
        layoutState1.addWidget(lblState1Contents)
        layoutState2.addWidget(lblState2Contents)
        layoutStateChange.addWidget(lblStateChangeContents)

        # Set the layouts to the group boxes
        self.grpState1.setLayout(layoutState1)
        self.grpState2.setLayout(layoutState2)
        self.grpStateChange.setLayout(layoutStateChange)

        # Add the group boxes to the main horizontal layout
        layout.addWidget(self.grpState1)
        layout.addWidget(self.grpState2)
        layout.addWidget(self.grpStateChange)

        # Set the main layout to the State Properties group box
        self._grp_StateProperties.setLayout(layout)
